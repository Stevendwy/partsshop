    ...
    <div class="o-content">
      ...
      <div class="o-total">
        <div class="ot-result">
          ...
          <span class="otr-freight">运费到付：{{carriage}}</span>
        </div>
        <div class="ot-detail">
          <span class="otd-status">{{orderStatus(order.status)}}</span>
          <span class="otd-order"
            @click="openDetail(order)">订单详情</span>
        </div>
        <div class="ot-operation">
          <el-button type="primary" class="oto-pay"
            @click="receive(order)">确认收货</el-button>
          <!-- <span class="close"
            @click="close(order)"
            v-if="order.status !== 'closed'">关闭交易</span> -->
        </div>
        <div class="ot-other">
  ...
  export default {
    ...
    methods: {
      ...
      this.$confirm('确认收货, 是否继续?', '提示', {
        ...
        this.handleOrder({order_id: order.order_id, status: this.type, action: 'receive'})
          .then(() => this.$message({
            message: '确认收货成功',
            type: 'success'
          }))
